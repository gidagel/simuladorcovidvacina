turtles-own
  [ sick?                ;; if true, the turtle is infectious
    ...
    delay-vacina         ;; time the vacine need to become effective
    eficacia-vac         ;; eficacia da vacina define a propabilidade do virus imunizar ou nao (esse numero depende de qual tipo da vacina)
    vacinado?            ;; tag para ja foi vacinado
]
...
globals
  [ infectado            ;; number os people infected
    ...
    immunity-vac   ;; how many weeks immunity after being vacinated
    periodo-de-transmissao-normal ;; for how long the person can
    numero-de-pessoas    ;; converte densidade absoluta para numero de pessoas considerando a grade 40x40!
    taxa-transmissao-idh     ;; recebe a taxa de transmissao com inpute de IDH e converte para porcentagem.
    chance-de-recuperacao-normal ;; defini a porcentagem de recuperacao da doenca
    num-vacinados         ;; total de vacinados  por dia
    porcentagem-de-confinados ;; variáveis que entram no modelo completo
    porcentagem-de-imunes ;;
    vacinados ;;
...
to get-older ;; turtle procedure
  ...
  if vacina? [set delay-vacina delay-vacina - 1 ]
end

to become-immune ;; turtle procedure
  set sick? false
  set sick-time 0
  set remaining-immunity (immunity-duration - remaining-immunity)
end

to become-yolk ;; turtle procedure vacinado
  set vacinado? true
  set delay-vacina immunity-vac
  set eficacia-vac 62
end
...
to cond-new-host
  if ticks mod reinfection-period = 0
    [ hatch 1 ;;@PATRICIA duas pessoas renfectadas
...
;; Se chegar no ultimo dia do delay da vacina e está saudavel e não está imune ainda, roda a roleta para se tornar imune
to cond-vacina
  if delay-vacina = 1 [ if  not sick?  [if random-float 100 < eficacia-vac [ become-immune ]]] ;;mudar o esquema do (or imune)
...
to infect ;; turtle procedure
  ask other turtles-here with [ not sick? and not immune? ] [if random-float 100 < taxa-transmissao-idh [get-sick]]

end


to recover-or-die ;; @HELIO: Alteracao onde pessoas vacinas nunca morrem
  if sick-time > periodo-de-transmissao-normal
[ifelse random-float 100 < chance-de-recuperacao-normal
 [become-immune] [ifelse vacinado? = false
[set numero-mortos numero-mortos + 1 die] [become-immune]]]
...
to setup-constants
  ...
  set periodo-de-transmissao-normal 18
  ;; IMUNIDADE
  if duração-imunidade-vacinal = "12-meses" [set immunity-duration 365] ;;   ;;set immunity-duration 365 ;; fixa em 1 ano
  if duração-imunidade-vacinal = "18-meses" [set immunity-duration 548] ;;
  if duração-imunidade-vacinal = "24-meses" [set immunity-duration 730] ;;
  set immunity-vac  67
  set numero-mortos 0
  set reinfection-period 14
  set reproduce-period 360 ;; quantidade de dias entre tentativas de reprodução
  set numero-de-pessoas 0.0498 * densidade-populacional
  set chance-de-recuperacao-normal 99.6
  ;; para ler a taxa de transmissao
  if IDH/IPC = "muito baixo" [set taxa-transmissao-idh 43]
  if IDH/IPC = "baixo" [set taxa-transmissao-idh 41]
  if IDH/IPC = "medio" [set taxa-transmissao-idh 39]
  if IDH/IPC = "alto" [set taxa-transmissao-idh 37]
  if IDH/IPC = "muito alto" [set taxa-transmissao-idh 36]
  ;; As variaveis que sao sliders dno modelo completo
  set porcentagem-de-confinados 27
  set porcentagem-de-imunes 30
  set vacinados 0.25 ;;taxa de vacinados por dia
end

;; We create a variable number of turtles of which 10 are infectious,
;; and distribute them randomly
to setup-turtles
  create-turtles numero-de-pessoas
    [ setxy random-xcor random-ycor
      ...
      set vacinado? false
      get-healthy ]
  ask n-of (numero-de-pessoas * (porcentagem-de-imunes / 100 )) turtles
    [ become-immune ]
  ask n-of 1 turtles    ;; number that begin infected
    [ get-sick] ;;AQUI É O LUGAR PRA INSERIR VARIANTES NOVAS
  ask n-of (numero-de-pessoas * (1 - porcentagem-de-confinados / 100)) turtles
    [get-free]
  ask n-of 1 turtles ;;(numero-de-pessoas * (vacinados / 100 )) turtles
    [become-yolk] ;;PODEMOS INICIAR COM DIFERENTES VACINAS
end

to update-global-variables
  if count turtles > 0
    [ set %infectadoTotal (  infectado / numero-de-pessoas) * 100
      set %immune (count turtles with [ immune? ] / numero-de-pessoas) * 100
      set %numero-mortos (  numero-mortos / numero-de-pessoas) * 100
  ]
end
...
to go
  ask turtles [
    ...
    if remaining-immunity = 0 and vacinado? = True [set vacinado? false] ;;@HELIO MINHA UNICA ALTERAÇÃO EM RELAÇÃO AO BUG DA VACINA FOI NESSE PONTO
  ]
  if ticks mod reinfection-period = 0 [
  ...
  ;;
  ;; Vacina como uma variavel
  ;;let num-vacinado count turtles with [ vacina? ]
  let para-vacinar ((count turtles) * (vacinados / 100) );; lidar com o fato de ser uma taxa pequena, como guardar a sobra? de uma divisao por inteiros? como quarda para nao perder ninguem...
  set num-vacinados  (num-vacinados + para-vacinar)
  let alpha (count turtles with [ not vacinado?])
  if num-vacinados > 1 and alpha >= 1 [
    let resto (num-vacinados - floor (num-vacinados) )
    ask up-to-n-of floor (num-vacinados) turtles with [ not vacinado? ] [become-yolk]
    set num-vacinados resto]
  ;;
  update-global-variables
  update-display
  tick
...
"" ""
PENS
"infectado" 1.0 0 -2674135 true "" "plot (count turtles with [ sick? ]) * 100 / numero-de-pessoas "
